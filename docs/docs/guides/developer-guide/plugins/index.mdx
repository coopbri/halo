...
The `graphql-tag` package is a dependency of the Vendure core package. Depending on the package manager you are using, you may need to install it separately with `bun add graphql-tag`.
...
```bash
bun run migration:generate wishlist-plugin
```
